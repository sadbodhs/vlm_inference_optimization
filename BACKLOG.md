...
| 8 | manufacturing domain (HA4M or InHARD) | user, 2026-09-24 | cropping helps more when people are large in frame | data + ~1 GPU day | B · R2 | published (R2): crops −7; model at the floor; data deleted |
...
| 10 | trained action detector baseline | reviewers will ask; R2: zero-shot VLM at the floor on assembly steps | trained wins on closed classes, VLM on open ones; on HA4M a trained recogniser is the only way off the floor | ~1 week | B · R4 | idea — raised by R2 |
...
| 16 | R1b: crops chosen without labels (tracker proximity groups), negatives from gate-fired windows, crop + context as lead arm | R1: oracle boxes leak participants; crops name an activity for 86–100% of uninvolved people | the single-person +12–16 point gain survives label-free crops; crop + context keeps false alarms near the full frame's | ~0.5 GPU day | B · R1b | published (R1b): mostly failed |
| 17 | precision pass: confirm a named activity (second question, or an easier "nothing notable") | R1/R1b: every crop arm raises false alarms 1.7–2.9× | halves crop false alarms at < 20% more tokens | ~0.5 GPU day | B | idea |

...
|---|---|---|---|---|---|---|
| 1 | actor-crop reference level (margin 1.2–3×, person 112/224/448 px, native) | E7d crop results; Context-Aware RCNN | an optimum margin of 1.5–2×; gains largest for small actors | ~1 GPU day | B · R1 | planned |
| 2 | marked full frame (box drawn around the actor) | red-circle paper | beats the plain full frame at zero token cost | small, inside R1 | B · R1 | planned |
| 3 | crop *plus* a small full frame (ViCrop-style) | ViCrop | recovers context-dependent actions the crop loses | small, inside R1 | B · R1 | planned |
| 4 | ideal crop from ground-truth boxes vs tracker boxes | tracker ceiling 59% / 31% | separates "the idea works" from tracker misses | small, inside R1 | B · R1 | planned |
| 5 | per-actor scoring, prompt adapted to crops | E7 scoring was per window | fair test of person-level actions | code only | B · R1 | planned |
